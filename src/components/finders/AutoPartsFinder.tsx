...
const AutoPartsFinder = ({ onFilterChange, stats, previewImage, attributes, onCompare }: FinderProps) => {
  return (
    <div className="space-y-4">
      {/* Dynamic Attributes Rendering */}
      {/* We can render specific attributes like 'condition' at the top if we want, 
          or just render everything in a grid. For now, let's render everything dynamically. */}
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        {/* Fallback/Hardcoded if attributes are empty to show UI structure (Optional, but user asked for dynamic) 
            If attributes are empty, the grid will be empty.
        */}
        {Object.keys(attributes).length === 0 && (
          <div className="col-span-2 text-center text-sm text-muted-foreground p-2">
            Select filters to see available attributes or no attributes found.
          </div>
        )}
      </div>
...
export default AutoPartsFinder;
